...
const UserSidebar = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-50 border-r flex flex-col h-screen transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          <Button variant={isActive("/dashboard") ? "secondary" : "ghost"} className="w-full justify-start gap-3" onClick={() => { navigate("/dashboard"); setIsOpen(false); }}>
            <LayoutDashboard size={18} /> Dashboard
          </Button>

          <Button variant={isTicketsActive() ? "secondary" : "ghost"} className="w-full justify-start gap-3" onClick={() => { navigate("/dashboard/tickets"); setIsOpen(false); }}>
            <Ticket size={18} /> My Tickets
          </Button>

          <Button variant={isNotificationsActive() ? "secondary" : "ghost"} className="w-full justify-between gap-3 group" onClick={() => { setUnreadCount(0); navigate("/dashboard/notifications"); setIsOpen(false); }}>
        ...
        <div className="p-4 mt-auto space-y-2 border-t">
          <Button variant={isSettingsActive() ? "secondary" : "ghost"} className="w-full justify-start gap-3" onClick={() => { navigate("/dashboard/settings"); setIsOpen(false); }}>
            <SettingsIcon size={18} /> Settings
          </Button>

          <Button variant="ghost" className="w-full justify-start gap-3 text-red-500 hover:bg-red-50" onClick={() => { logout(); navigate("/"); setIsOpen(false); }}>
